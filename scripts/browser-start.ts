...
if (
  useProfile &&
  args[profileIndex + 1] &&
  !args[profileIndex + 1].startsWith("-")
) {
...
const unknownArg = args.find((a) =>
  !a.startsWith("-")
    ? !validArgs.includes(a)
    : !["--profile", "--force"].includes(a),
);
if (unknownArg) {
...
function killSkillChrome() {
  try {
    if (process.platform === "darwin") {
      ...
      execSync(`taskkill /F /FI "COMMANDLINE eq *user-data-dir=${BASE_DIR}*"`, {
        stdio: "ignore",
      });
    } else {
      execSync(`pkill -f "user-data-dir=${BASE_DIR}"`, { stdio: "ignore" });
...
if (!chromePath) {
  console.error(
    JSON.stringify({ error: "Chrome not found. Install Google Chrome." }),
  );
  process.exit(1);
}
...
if (!forceRestart) {
  ...
  if (alreadyRunning) {
    ...
    if (currentMode === requestedMode) {
      ...
      console.log(
        `Chrome already running on :9222 (reusing ${currentMode}, ${dir})`,
      );
      process.exit(0);
    }
...
if (useProfile) {
  ...
  try {
    ...
    console.error(
      JSON.stringify({
        error: "rsync not found. Required for --profile option.",
      }),
    );
    process.exit(1);
  }
  ...
  if (!profileName) {
    ...
    if (!profileName) {
      console.error(
        JSON.stringify({
          error: `No Chrome profiles found in: ${chromeDataDir}`,
        }),
      );
      process.exit(1);
    }
  ...
  if (!existsSync(profileSource)) {
    console.error(
      JSON.stringify({
        error: `Chrome profile "${profileName}" not found at: ${profileSource}`,
      }),
    );
    process.exit(1);
  }
  ...
  try {
    execSync(
      `rsync -a --delete ${rsyncExcludes} "${profileSource}/" "${targetProfile}/"`,
      {
        stdio: "pipe",
      },
    );
    ...
    console.error(
      JSON.stringify({
        error: `Failed to sync Chrome profile "${profileName}"`,
      }),
    );
    process.exit(1);
  }
...
if (!connected) {
  console.error(
    JSON.stringify({ error: "Chrome started but failed to connect on :9222" }),
  );
  process.exit(1);
}
